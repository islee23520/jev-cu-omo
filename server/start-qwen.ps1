...
if (-not $Listener) {
  $Server = Start-Process -FilePath $Ollama -ArgumentList 'serve' -PassThru -WindowStyle Hidden -RedirectStandardOutput "$Root\server.log" -RedirectStandardError "$Root\server-error.log"
  $Server.Id | Set-Content "$Root\server.pid"
}
& $Ollama pull 'qwen3:8b' *> "$Root\pull.log"
$Code = $LASTEXITCODE
$Code | Set-Content "$Root\pull.exit"
exit $Code
